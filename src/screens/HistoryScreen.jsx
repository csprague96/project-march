      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <header className="flex items-center justify-between gap-4">
          ...
          <div className="text-right">
            <div className="text-xs uppercase tracking-[0.2em] text-[var(--text-secondary)]">Saved cards</div>
            <div className="mt-1 text-sm text-[var(--text-primary)]">
              {records.length} total {isSyncing ? '· syncing queue' : ''}
            </div>
          </div>
        </header>
        ...
        {records.length ? (
            ...
            {records.map((record) => (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  >
                    {record.triage_category ?? 'Unclassified'}
                  </span>
                </div>
          ...
          <div className="rounded-3xl border border-dashed border-[var(--border)] bg-[var(--bg-card)] p-8 text-center text-[var(--text-secondary)]">
            No saved triage cards yet. Capture a card, review the result, then save it here.
          </div>
        )}
      </div>
    </section>
